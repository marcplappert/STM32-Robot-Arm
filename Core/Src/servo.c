...
void terminal_print_servo_data(ServoArm_t *arm) {
	...
	if (terminal_print_count == TERMINAL_PRINT_CYCLE_COUNT) {
		...
		for (int id = 1; id <= arm->servo_count; id++) {
			uint16_t Location = (arm->servo_buffer[id][6] << 8) | arm->servo_buffer[id][5];
			int16_t Speed = (arm->servo_buffer[id][8] << 8) | arm->servo_buffer[id][7];
			int16_t raw_load = (arm->servo_buffer[id][10] << 8) | arm->servo_buffer[id][9];
			float Load = (float) raw_load / 10;
			uint8_t Temp = arm->servo_buffer[id][12];
			uint8_t Status = arm->servo_buffer[id][14];
			uint8_t Motion = arm->servo_buffer[id][15];
			uint8_t raw_voltage = arm->servo_buffer[id][11];
			float Voltage = (float) raw_voltage / 10;
			int8_t raw_current = arm->servo_buffer[id][18];
			float Current = (float) raw_current * 6.5 / 1000;
			int text_len = snprintf(msg, sizeof(msg),
							...
							id, Location, Speed, Load, Temp, Status, Motion, Voltage, Current);
...
void servo_write_byte(ServoArm_t *arm, uint8_t id, uint8_t start_addr, uint8_t num_bytes, ...) {
	...
	HAL_UART_Transmit(arm->huart, servo_cmd, num_bytes + 7, HAL_MAX_DELAY);
}
...
void servo_read_id(ServoArm_t *arm, uint8_t id){
	...
	//wait till all motors have answered
		uint32_t start_time = HAL_GetTick();
		while(arm->servo_buffer_ready[id] == 0){
			if((HAL_GetTick() - start_time) > SERVO_MAX_WAIT_TIME){
...
void servo_set_angle(ServoArm_t *arm, uint8_t id, float angle){
	...
	servo_write_byte(arm, id, 0X2A, 6, steps & 0xFF, (steps >> 8) & 0xFF, 0X00, 0X00, speed & 0xFF, (speed >> 8) & 0xFF);
}
...
void servo_zero_point_calibration(ServoArm_t *arm){
	...
	//unlock servos
	servo_write_byte(arm, 0xFE, 0x37, 1, 0);
	servo_write_byte(arm, 0xFE, 0x28, 1, 0);
	...
	for(uint8_t id = 1; id < 7; id++){
		...
		HAL_Delay(20);
		servo_write_byte(arm, id, 0x28, 1, 0x80);
	...
	//lock servos
	servo_write_byte(arm, 0xFE, 0x37, 1, 1);
	servo_write_byte(arm, 0xFE, 0x28, 1, 1);
...
void servo_min_max_calibration(ServoArm_t *arm){
	...
	//calibrate servos
	for(uint8_t id = 1; id <= arm->servo_count; id++){
		...
		min_angle = 4095;
		max_angle = 0;
		servo_write_byte(arm, id, 0x09, 2, (uint8_t)(final_min_angle & 0xFF), (uint8_t)((final_min_angle >> 8) & 0xFF));
		servo_write_byte(arm, id, 0x0B, 2, (uint8_t)(final_max_angle & 0xFF), (uint8_t)((final_max_angle >> 8) & 0xFF));
		HAL_UART_Transmit(&huart2, (uint8_t*)"Calibrated\r\n", 12, HAL_MAX_DELAY);
		HAL_Delay(20);
		while(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET){}
		HAL_Delay(20);
...
void servo_max_torque_initialization(ServoArm_t *arm){
	servo_write_byte(arm, 0xFE, 0x37, 1, 0);
	servo_write_byte(arm, 0xFE, 0x10, 2, 0x12, 0x01);
